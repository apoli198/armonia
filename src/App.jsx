...
// ─── Normalize garment entry (migration from old string format) ───────────────
...
// ─── useDarkMode — always reads system on mount, toggle overrides for session ─
function useDarkMode(){
 // Always start from system preference, ignore saved value
 const sysDark=()=>window.matchMedia?.("(prefers-color-scheme: dark)").matches||false;
 const[dark,setDark]=useState(sysDark);

 useEffect(()=>{
   // Sync on mount in case useState ran before matchMedia was ready
   setDark(sysDark());
...
// ─── ColorPickerModal ─────────────────────────────────────────────────────────
const CANVAS_W=300,CANVAS_H=225;
...
function ColorPickerModal({value,onClose,onChange,savedColors,onSave}){
 ...
 // Convert clientX/clientY → canvas CSS coords
 const toCss=useCallback((clientX,clientY)=>{
   const cv=canvasRef.current;if(!cv)return{x:0,y:0};
   const rect=cv.getBoundingClientRect();
   // rect is in viewport coords; clientX/Y are also viewport coords — no scroll needed
   return{
 ...
 const pick=useCallback((cssX,cssY)=>{
   ...
   const ctx=cv.getContext("2d");
   let R=0,G=0,B=0,n=0;
   // Sample 5×5 at buffer resolution; cssX/Y already clamped 0..CANVAS_W/H
           ...
           <div>
             {!imgSrc?(
               ...
               <div>
                 ...
                 {/* Fixed-size container — canvas always CANVAS_W×CANVAS_H CSS px */}
                 <div style={{width:CANVAS_W,height:CANVAS_H,margin:"0 auto",position:"relative",borderRadius:14,overflow:"hidden",border:T.dark?"1.5px solid rgba(255,255,255,0.12)":"1.5px solid rgba(0,0,0,0.1)",touchAction:"none",background:"#000"}}>
                   <canvas
                     ref={canvasRef}
                     style={{display:"block",cursor:"crosshair",userSelect:"none",WebkitUserSelect:"none",
                       // canvas element sized exactly to container — no CSS scaling distortion
...
export default function App(){
 ...
 const[fixedColors,setFixedColors]=useState(()=>{
   ...
   // Migrate: normalize every entry regardless of old/new format
