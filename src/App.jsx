...
// HEX utente → normalizzato verso range realistico tessuti (~0-62)
// Questo è il parametro principale: tutti i calcoli usano il valore normalizzato
...
// ─── Season fit ranges (fabric-sat) ──────────────────────────────────────────
const SEASON_SAT_RANGES={
 "spring-true":  {min:25,max:55},"spring-light":{min:18,max:48},"spring-warm":{min:28,max:58},
 "summer-true":  {min:12,max:40},"summer-light":{min:10,max:36},"summer-soft":{min:6,max:28},
 "autumn-true":  {min:20,max:52},"autumn-warm": {min:25,max:55},"autumn-deep":{min:22,max:55},
 "winter-true":  {min:30,max:60},"winter-cool": {min:28,max:58},"winter-bright":{min:40,max:62},
};
const SEASON_LIGHT_RANGES={
 "spring-true":  {min:35,max:82},"spring-light":{min:42,max:90},"spring-warm":{min:32,max:80},
 "summer-true":  {min:35,max:82},"summer-light":{min:42,max:88},"summer-soft":{min:38,max:84},
 "autumn-true":  {min:22,max:68},"autumn-warm": {min:25,max:70},"autumn-deep":{min:12,max:58},
 "winter-true":  {min:18,max:78},"winter-cool": {min:15,max:75},"winter-bright":{min:20,max:88},
};
...
// Calcola season level (0-3) per un singolo capo
function evaluateSeasonLevel(hex,season,comboHexes=[],material="normal",weight=25){
 ...
 if(s<10){
   ...
   // Peso visivo basso → attenua il segnale
...
// Valuta accostamenti tra capi (flag tipo 2)
// Restituisce array di level 0-3, uno per item
// Il flag va sempre al capo con peso visivo minore
function evaluateComboFits(items){
 ...
 for(let i=0;i<items.length;i++){
   for(let j=i+1;j<items.length;j++){
     ...
     let problem=0;
     // 1. Due neutrali scuri simili (nero+navy, nero+grigio scuro)
     ...
     // 2. Pseudo-match: hue vicini ma saturazione molto diversa
     ...
     // 3. Clash temperatura: warm saturo + cool saturo
...
// ─── Profile analysis ─────────────────────────────────────────────────────────
// Undertone calcolato da hue HSL dell'hex normalizzato (screenToFabricSat),
// non da canali RGB grezzi. La foto è solo strumento di selezione dell'hex —
// il calcolo usa sempre l'hex come scelto dall'utente.
//
// Pelli umane: H∈[0°,50°]
//   H ~0-15°  = rosato/viola → cool
//   H ~20-30° = beige/neutro
//   H ~35-50° = dorato/arancio → warm
// Neutro calibrato a H=22°
function analyzeProfile(skin,eyes,hair,reflexes){
 ...
 // Undertone pelle: hue-based, normalizzato su range pelli reali
 const skinHueValid=sSk>4;
 let utScore=skinHueValid?(hSk-22)/22:0;
 utScore=Math.max(-1.5,Math.min(1.5,utScore));

 // Contributo occhi: cluster recognition (non hue grezzo)
 let eyeUt=0;
 if(sEy>18){
   if(hEy>75&&hEy<170)      eyeUt=-0.15; // verde → neutro-freddo
   else if(hEy>170&&hEy<250)eyeUt=-0.25; // azzurro/grigio-blu → freddo
   else if(hEy<45||hEy>320) eyeUt=+0.18; // nocciola/amber → warm
 }else{
   eyeUt=lEy<25?0:-0.12; // occhio scuro desaturato=neutro, grigio chiaro=liev. freddo
 }

 // Contributo capelli: solo se non quasi-neri (L>18) e abbastanza saturi
 // Capelli neri puri → solo depth, niente undertone
 let hairUt=0;
 if(lHr>18&&sHr>8){
   hairUt=(hHr-30)/40; // 30°=neutro capelli
   hairUt=Math.max(-1,Math.min(1,hairUt))*0.3;
 }

 // Score composito: pelle 60%, occhi 25%, capelli 15%
 utScore=utScore*0.60+eyeUt*0.25+hairUt*0.15;

 // Reflex additivi sulla stessa scala normalizzata
 const RUT={skin:{warm:+0.45,cool:-0.45,neutral:0},eye:{golden:+0.18,green:-0.08,grey:-0.20},hair:{copper:+0.25,golden:+0.15,ashy:-0.20}};
 ...
 // Soglie asimmetriche: bias umano verso warm
 const undertone=utScore>0.12?"warm":utScore<-0.18?"cool":"warm";

 // Depth: capelli pesano di più (tratto più scuro)
 const depthScore=100-(lSk*0.55+lHr*0.35+lEy*0.10);
 const depth=depthScore>45?"deep":"light";

 // Intensity: occhi pesano di più, aggiustati da reflex
...
function detectSeason(skin,eyes,hair,reflexes){
 ...
 if(undertone==="warm"&&depth==="light"){
   base="spring";sub=intensityLevel==="low"?"light":intensityLevel==="high"?"warm":"true";
 }else if(undertone==="warm"&&depth==="deep"){
   base="autumn";sub=intensityLevel==="high"?"deep":intensityLevel==="medium"?"warm":"true";
 }else if(undertone==="cool"&&depth==="light"){
   base="summer";sub=intensityLevel==="low"?"soft":intensityLevel==="high"?"true":"light";
 }else{
   base="winter";sub=intensityLevel==="low"?"cool":intensityLevel==="high"?"bright":"true";
 }
...
function buildHarmonyPool(type,anchorH,profile,jitter=0){
 ...
 // Hue step ridotto per warm: evita drift verso hue freddi
...
function OutfitCard({combo,season}){
 ...
 const seasonFits=combo.items.map(item=>evaluateSeasonLevel(item.hex,season,combo.items.filter(i=>i.id!==item.id).map(i=>i.hex),item.material,item.weight));
 const comboFits=evaluateComboFits(combo.items);
...
function WardrobeTab({modes,setModes,fixedColors,setFixedColors,savedGarment,onSaveGarment,season}){
 ...
 // Contesto degli altri capi fissi per valutazione contestuale
 const fixedComboHexes=useCallback((gid)=>GARMENTS
   .filter(g=>g.id!==gid&&modes[g.id]==="fixed")
   .map(g=>normalizeEntry(fixedColors[g.id]).hex)
 ,[modes,fixedColors]);
 ...
 return(
   <div style={{padding:"1rem"}}>
     {GARMENTS.map(g=>{
       ...
       const fitLevel=mode==="fixed"?evaluateSeasonLevel(fixed.hex,season,fixedComboHexes(g.id),fixed.material,weight):0;
       return(
